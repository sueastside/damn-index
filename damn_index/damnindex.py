...
class DAMNIndex(object):
    ...
    def _serialize_asset_description(self, file_descr, asset_descr):
        ...
        return {
            ...
            'metadata': [{'key': key}
                         for key, meta in list(asset.metadata.items())] if asset.metadata else [],
            'dependencies': [{'subname': dep.subname,
                              'mimetype': dep.mimetype,
                              'file__filename': dep.file.filename,
                              'file__hash': dep.file.hash}
                             for dep in asset.dependencies] if asset.dependencies else [],
        }
